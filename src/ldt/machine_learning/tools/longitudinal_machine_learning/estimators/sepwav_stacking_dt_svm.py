from __future__ import annotations

from ldt.machine_learning.tools.standard_machine_learning.estimators.svm import (
    SVMEstimatorTemplate,
)
from ldt.utils.metadata import ComponentMetadata

from .base import LongitudinalEstimatorTemplate, make_standard_primitive_builder


class SepWavStackingDTSVMLongitudinalEstimatorTemplate(LongitudinalEstimatorTemplate):
    """Template definition for `sepwav_stacking_dt__svm` longitudinal estimator."""

    metadata = ComponentMetadata(
        name="sepwav_stacking_dt__svm",
        full_name="SepWav (Stacking + Decision Tree) + Support Vector Machine",
        abstract_description=(
            "[Longitudinal] Train one base estimator per wave and stack with Decision Tree. Base estimator: "
            "Support Vector Machine."
        ),
    )
    strategy_key = "sepwav_stacking_dt"
    base_estimator_key = "svm"
    _builder = make_standard_primitive_builder(
        primitive_key="sepwav_stacking_dt",
        standard_template=SVMEstimatorTemplate,
    )
