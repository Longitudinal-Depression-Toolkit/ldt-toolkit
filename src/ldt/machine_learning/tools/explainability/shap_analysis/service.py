...
class SHAPAnalysisRunner:
    """Library runner for SHAP explainability on saved model artefacts."""
    ...
    def run(
        ...
        *,
        ...
        """Execute SHAP analysis and export figures + report artefacts.
    ...
    def _build_report_text(
        *,
        ...
        lines = [
            ...
            "====================",
            f"Model artefact: {model_path.resolve()}",
    ...
    def _validate_model_path(path: Path) -> None:
        """Validate model artefact path."""

        if not path.exists() or not path.is_file():
            raise InputValidationError(f"Model artefact path does not exist: {path}")
        if path.suffix.lower() != ".pkl":
            raise InputValidationError(
                "Model artefact path must point to a `.pkl` file."
            )
