...
@beartype
class MeanImputer(MissingImputer):
    """Impute missing numeric values with each column mean.

    Runtime parameters:
        - no extra parameters are required
        - numeric columns are imputed independently using their own mean
    ...
    metadata = ComponentMetadata(
        ...
        abstract_description="Fill missing numeric values with each column mean.",
    )
    ...
    @beartype
    def fit(self, *, input_path: Path, output_path: Path, **kwargs: Any) -> MeanImputer:
        """Validate and store mean-imputation configuration.
        ...
        Returns:
            MeanImputer: The fitted imputer instance.
        """

        if kwargs:
            unexpected = ", ".join(sorted(kwargs))
            raise InputValidationError(
                f"Mean imputation does not accept extra parameters: {unexpected}"
            )
    ...
    def impute(
        ...
        """Execute mean imputation and write the imputed CSV output.
        ...
        if self._config is None:
            raise InputValidationError(
                "No mean-imputation configuration provided. Call `fit(...)` before `impute(...)`."
            )
        ...
        if missing_before > 0 and not eligible_columns:
            raise InputValidationError(
                "Mean imputation requires at least one numeric column with missing values."
            )
