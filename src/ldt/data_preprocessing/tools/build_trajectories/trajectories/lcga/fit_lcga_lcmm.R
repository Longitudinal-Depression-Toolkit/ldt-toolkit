...
# -----------------------------------------------------------------------------
# LCGA — R implementation as there is no Python package for longitudinal latent class analysis.
#
# Purpose:
#   Fit a latent class growth analysis model from long-format data and export
#   standardised artefacts consumed by the Python `LCGA` builder:
#     - assignments.csv
...
#   --n-trajectories --max-iter --n-init [--seed]
# -----------------------------------------------------------------------------
...
parse_args <- function(args) {
  if (length(args) == 0) {
    stop_with("No arguments passed to LCGA bridge.")
  }
  if ((length(args) %% 2) != 0) {
...
#   2) Fit LCGA model with `hlme` (and `gridsearch` for random starts).
...
main <- function() {
  ...
  random_formula <- stats::as.formula("~ -1")
