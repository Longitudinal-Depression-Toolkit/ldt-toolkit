...
def run_benchmark_longitudinal_ml_tool(
    *,
    ...
    """Run longitudinal-machine-learning benchmark workflows on temporal features.

    This tool benchmarks longitudinal estimators that require explicit temporal
    structure (`feature_groups` and `non_longitudinal_features`). It supports
    capability discovery (`list_estimators`, `list_metrics`) and full benchmark
    execution (`run_benchmark`) with artifacts and optional notebook output.
    ...
    Total default longitudinal benchmark roster: 47 estimators
    (42 wrapper+base combinations and 5 native longitudinal estimators).

    Supported techniques:

    | Technique | What it does |
    | --- | --- |
    | `list_estimators` | Returns the current longitudinal estimator roster with keys and descriptions. |
    ...
    | --- | --- | --- |
    | Binary classification | Yes | Runs one benchmark on the encoded binary target. |
    | Multi-class classification | Yes (via one-vs-rest) | Builds one binary scenario per class and runs a benchmark for each scenario. |
    | Single-class target | No | Raises `InputValidationError`. |

    For multi-class targets, one-vs-rest scenarios are generated with
    `TargetScenarioPlanner` and validated before training. The output contains
    one run entry per scenario with independent artifacts.
    ...
    Returns:
        dict[str, Any]: Scenario benchmark outputs and artifact paths for
            execution mode; catalog metadata for listing modes.

    Examples:
        ```python
        from ldt.machine_learning.tools.benchmark_longitudinal_ml.run import run_benchmark_longitudinal_ml_tool

        result = run_benchmark_longitudinal_ml_tool(
    ...
    return run_with_validation(
        lambda: _run_benchmark_longitudinal_ml_tool(technique=technique, params=params)
    )
